    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-2xl">
          ...
          <div className="mb-8 backdrop-blur-xl bg-white/10 rounded-2xl p-4 border border-white/20 shadow-xl">
          ...
          <div className="backdrop-blur-2xl bg-white/10 rounded-3xl border border-white/20 shadow-2xl p-8 mb-6 transition-all duration-300 hover:shadow-cyan-500/20 hover:border-white/30">
            {/* Question Kode Badge */}
            <div className="inline-flex items-center backdrop-blur-xl bg-gradient-to-r from-[#39a0c9]/30 to-[#7dcbe1]/30 border border-white/30 px-4 py-2 rounded-full mb-4 shadow-lg">
              <span className="text-white font-bold text-sm tracking-wider">{question.kode}</span>
            </div>
            ...
            {/* Scale Type Info */}
            <div className="backdrop-blur-xl bg-[#7dcbe1]/20 border-l-4 border-[#7dcbe1] rounded-lg p-4 mb-8 shadow-lg">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center">
                      {/* Radio Indicator */}
                      <div className={`
                        relative w-7 h-7 rounded-full transition-all duration-300
                        ${isSelected
                          ? 'bg-white shadow-lg'
                      ...
                      `}>
                        {isSelected && (
                          <div className="absolute inset-0 flex items-center justify-center">
                            <div className={`w-3 h-3 rounded-full bg-gradient-to-br ${option.gradient}`}></div>
                          </div>
                        )}
                      </div>
            ...
            {question.skala_detail && (
              <details className="mt-6 backdrop-blur-xl bg-white/5 rounded-xl border border-white/10 overflow-hidden">
                <summary className="px-4 py-3 text-sm text-white/80 cursor-pointer hover:bg-white/10 transition-colors font-medium">
                  💡 Lihat penjelasan detail skala
                </summary>
                <div className="px-4 py-4 bg-white/5 text-sm text-white/70 whitespace-pre-line border-t border-white/10">
                  {question.skala_detail}
                </div>
              </details>
            )}
          ...
          <div className="flex gap-4">
            <button
              ...
              className={`
                flex-1 py-4 rounded-2xl font-bold transition-all duration-300
                backdrop-blur-xl border-2
                ${currentStep === 1
                  ? 'bg-white/5 text-white/30 border-white/10 cursor-not-allowed'
                  : 'bg-white/10 text-white border-white/30 hover:bg-white/20 hover:border-white/50 hover:shadow-lg hover:scale-[1.02]'
                }
              `}
            >
              ← Kembali
          ...
          <div className="mt-8 text-center">
            <p className="text-white/50 text-sm">Sekolah Literasi Indonesia</p>
          </div>
        </div>
      </div>
